...
## ⚠️ トラブルシューティング (MQTT通信がうまくいかない場合)

**Q. YOLOやUI画面が「Connected（接続済）」になっているのに、Python（パトロール隊）側でログが出ない・ボタンが効かない**

これはPCのバックグラウンド（Windowsサービスなど）で **別のMQTTブローカー（Mosquitto等）がすでに動作している** 場合に発生する、典型的な「パラレルワールド（接続先の分裂）」現象です。

*   **原因:** 
    Node-RED内蔵の `Aedes` ブローカーがポート `1883` を使おうとしますが、すでに `Mosquitto` 等が占有していると起動に失敗します。
    このとき、`Mosquitto` が `localhost` (`127.0.0.1`) のみで待ち受ける設定になっていると、Pythonプログラム（`MQTT_BROKER = "localhost"`）は `Mosquitto` に接続されてしまいます。
    一方で、外部のYOLOやNode-REDのUIノードはWi-FiのIPアドレス（例: `192.168.11.2`）を指定して接続するため、うまくAedes等の別ブローカーに繋がってしまい、**「全員繋がっているのにお互いの通信が見えない」** 状態になります。
*   **解決策:**
    1. 最も確実な方法は、Windowsのサービス等で動いている不要な `Mosquitto` を停止・無効化することです。
    2. または、Python側の `config.py` 内の `MQTT_BROKER` を `localhost` から **「現在PCが繋がっているWi-FiのIPアドレス（例: 192.168.11.2）」** に明示的に書き換えることで、全員を同じブローカーに強制的に合流させることができます。
    ※ただし、後者の場合はWi-Fiネットワークが変わるたびにIPアドレスの書き換えが必要になるため注意してください。
